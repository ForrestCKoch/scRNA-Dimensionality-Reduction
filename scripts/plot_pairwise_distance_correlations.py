...
if __name__ == '__main__':
    z = pd.read_csv('data/results/internal_measures_reduced.csv').dropna()
    ...
    for ivm in ['ss_euc','ss_seu','ss_cor','ss_cos','vrc','dbs']:
        ...
        plt.clf()
        X.to_csv('data/results/pw_correlations-'+ivm+'.csv')

    d = {'dataset':[],'method':[],'ss_euc':[],'vrc':[],'dbs':[]}#,acc:[]}
    for group in grouped.groups.keys():
        data = pd.DataFrame(grouped.get_group(group))
        d['dataset'].append(group[0])
        d['method'].append(group[1])
        ...
        d['dbs'].append(data['rho.euclidean'].iloc[np.argmax(data['dbs'])])
        #ari = np.max(data[acc])

    X = pd.DataFrame(d).groupby('method').mean()
    pd.DataFrame(d).to_csv('data/results/pw_correlations_by_best_ivm.csv')
