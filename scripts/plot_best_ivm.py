...
for d in datsets:
    for m in methods:
        ...
        if m == 'lda' or m=='icm' or 'vpac':
            file_to_show =  os.path.join('data/embeddings',d,count_type,m,'4.pkl')
        else:
        ...
        axes[idx].scatter(show_dat.values[:,1],show_dat.values[:,2],c=LabelEncoder().fit_transform(show_dat.values[:,0]),s=1)
