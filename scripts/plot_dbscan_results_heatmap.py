...
if __name__ == '__main__':
    ...
    for group in grouped.groups.keys():
        ...
        d['ss'].append(data[acc].loc[np.argmax(data['ss'])])
        d['vrc'].append(data[acc].loc[np.argmax(data['vrc'])])
        d['dbs'].append(data[acc].loc[np.argmin(data['dbs'])])
        #ari = np.max(data[acc])
    ...
    X.to_csv('data/results/optimal_dbscan_trials_summarized_'+metric+'_'+acc+'.csv')
